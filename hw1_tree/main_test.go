...
func BenchmarkDirTree(b *testing.B) {
	/*
		go test -bench .
		...
		Windows:
		go tool pprof hw1_tree.test.exe cpu.out
		go tool pprof hw1_tree.test.exe mem.out

		Linux:
		go tool pprof hw1_tree.test cpu.out
		go tool pprof hw1_tree.test mem.out
	...
	*/
	for i := 0; i < b.N; i++ {
		_ = dirTree(new(bytes.Buffer), "testdata", false)
	}
}
